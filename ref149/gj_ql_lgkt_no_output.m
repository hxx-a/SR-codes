function [x,y]=gj_ql_lgkt(a,b,r,n,h,s)
% 耦合方式全连接耦合
...
for i=1:length(s)-1
    ...
    k1_w=(b*x(i)-w(i));
    for j=1:n
    m_y(j,1)=(-y(j,i)^3+(a+1)*y(j,i)^2-a*y(j,i)-W(j,i)+r*(x(i)+sum(y(:,i))-(n+1)*y(j,i)));
    ...
    for j=1:n
    m_y(j,2)=(-(y(j,i)+m_y(j,1)*h/2)^3+(a+1)*(y(j,i)+m_y(j,1)*h/2)^2-a*(y(j,i)+m_y(j,1)*h/2)-(W(j,i)+m_w(j,1)*h/2)+r*((x(i)+k1_x*h/2)+sum(y(:,i))+sum(m_y(:,1))*h/2-(n+1)*(y(j,i)+m_y(j,1)*h/2)));
    ...
    for j=1:n
    m_y(j,3)=(-(y(j,i)+m_y(j,2)*h/2)^3+(a+1)*(y(j,i)+m_y(j,2)*h/2)^2-a*(y(j,i)+m_y(j,2)*h/2)-(W(j,i)+m_w(j,2)*h/2)+r*((x(i)+k2_x*h/2)+sum(y(:,i))+sum(m_y(:,2))*h/2-(n+1)*(y(j,i)+m_y(j,2)*h/2)));
    m_w(j,3)=(b*(y(j,i)+m_y(j,2)/2)-(W(j,i)+m_w(j,2)/2));
    end
    ...
    for j=1:n
    m_y(j,4)=(-(y(j,i)+m_y(j,3)*h)^3+(a+1)*(y(j,i)+m_y(j,3)*h)^2-a*(y(j,i)+m_y(j,3)*h)-(W(j,i)+m_w(j,4)*h)+r*((x(i)+k3_x*h)+sum(y(:,i))+sum(m_y(:,3))*h-(n+1)*(y(j,i)+m_y(j,3)*h)));
